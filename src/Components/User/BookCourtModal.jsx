...
function BookCourtModal({ court, onClose }) {
  ...
  useEffect(()=>{
    ...
    const fetchTimeSlots = async()=>{
      try{
      const fetchedTimeSlots = await api.get(`/user/time-slots/${court.id}/${booking.booking_date}`)
      ...
      <div className="form-section">
        <form onSubmit={handleBooking} action="">
          <div className="input-wrapper">
            ...
            <input value={booking.booking_date} onChange={(e)=>setBooking({...booking, booking_date: e.target.value})} type="date" required />
          </div>
          <div className="input-wrapper">
            <label htmlFor="">Choose a Time Slot</label>
            <select value={booking.time_slot_id} onChange={(e)=>setBooking({...booking, time_slot_id: e.target.value})} name="" id="" required>
              <option value="">Choose a time slot</option>
              {timeSlots.map((timeSlot)=>(
                <option key={timeSlot.id} value={timeSlot.id}>{timeSlot.start_time.slice(0, 5)} - {timeSlot.end_time.slice(0, 5)}</option>
              ))
              }
            </select>
          </div>
